...
impl GameScene {
    ...
    fn draw_light_map(&self, state: &mut SharedGameState, ctx: &mut Context) -> GameResult {
        ...
        let base_color = match self.lighting_mode {
            LightingMode::Ambient => Color::from_rgb(10, 10, 10),        // 全黑
            LightingMode::BackgroundOnly => Color::from_rgb(100, 100, 110), // 淡灰
            _ => Color::from_rgb(100, 100, 110),
        };

        graphics::clear(ctx, base_color);
...

        {
                ...
                [(&self.player1, &self.inventory_player1), (&self.player2, &self.inventory_player2)].iter()
            {
                if player.cond.alive() && !player.cond.hidden(){
                    // 强制黑暗场景一直显示角色光环
                    if self.lighting_mode==LightingMode::Ambient{
                        self.draw_light(
                            interpolate_fix9_scale(
                ...
                if player.cond.alive() && !player.cond.hidden() && inv.get_current_weapon().is_some() {
                    ...
                    if state.settings.light_cone {
                        ...
                        let base_attr = match self.lighting_mode {
                            LightingMode::Ambient => 0.05,        // 全黑
                            LightingMode::BackgroundOnly => 0.0, // 淡灰
                            _ => 0.0,
                        };

                        let (color, att) = match inv.get_current_weapon() {
                            Some(Weapon { wtype: WeaponType::Fireball, .. }) => ((180u8, 80u8, 0u8), 0.85+base_attr),
                            Some(Weapon { wtype: WeaponType::PolarStar, .. }) => ((140u8, 140u8, 160u8), 0.83+base_attr),
                            Some(Weapon { wtype: WeaponType::Spur, .. }) => ((160u8, 160u8, 210u8), 0.87+base_attr),
                            Some(Weapon { wtype: WeaponType::SuperMissileLauncher, .. }) => ((200u8, 170u8, 80u8), 0.88+base_attr),
                            Some(Weapon { wtype: WeaponType::Snake, .. }) => ((250u8, 170u8, 100u8), 0.85+base_attr),
                            Some(Weapon { wtype: WeaponType::Nemesis, .. }) => ((150u8, 170u8, 250u8), 0.88+base_attr),
                            Some(Weapon { wtype: WeaponType::Blade, .. }) => continue 'cc,
                            _ => ((150u8, 150u8, 150u8), 0.9),
                        };
